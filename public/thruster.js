...
window.addThruster = player=>{
  ...
  for(let i=0;i<4;i++){
    ...
    e.setAttribute('rain',`width:0.5;height:8;splash:false;color:${window.config.avatar.models[player.faceIndex].thruster.outerColor};dropRadius:0.1,vector:0,-15,0;count:500`);
    e.setAttribute('rotation',r1[i]);
    e.setAttribute('class','outer');
    thruster.appendChild(e);
  }
  for(let j=0;j<4;j++){
    let e = document.createElement('a-entity');
    e.setAttribute('rain',`width:0.5;height:6;splash:false;color:${window.config.avatar.models[player.faceIndex].thruster.innerColor};dropRadius:0.3,vector:0,-15,0;count:100`);
    e.setAttribute('rotation',r2[j]);
    e.setAttribute('class','inner');
    thruster.appendChild(e);
  }
...
window.updateThruster = player=>{
  let thruster = player.thruster;
  let t = window.config.avatar.models[player.faceIndex].thruster;
  thruster.querySelectorAll('.outer').forEach(e=>{
   e.setAttribute('rain',`color:${t.outerColor}`);
  });
  thruster.querySelectorAll('.inner').forEach(e=>{
   e.setAttribute('rain',`color:${t.innerColor}`);
  });
}